...
# Note: yfinance, yahooquery, nsetools, investpy are imported inside tools to handle failures gracefully
...
def get_company_fundamentals(symbol: str, data_point: str = "all") -> dict:
    ...
    Priority: YahooQuery (TTM/Annual) → yfinance fallback.
...
def get_news_headlines(company_name: str) -> dict:
    ...
    # --- SOURCE 1: NewsAPI (Best Quality, Limited Requests) ---
    try:
        from newsapi import NewsApiClient
        import datetime
    ...
    # --- SOURCE 3: investpy (Scraper - High Failure Rate but good data) ---
    if not headlines:
        try:
            import investpy
            clean_sym = clean_symbol(company_name)
            # investpy requires the exact symbol or name. We try the symbol first.
